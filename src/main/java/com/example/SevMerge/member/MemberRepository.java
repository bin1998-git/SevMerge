...
public interface MemberRepository extends JpaRepository<Member, Long> {
    ...
    @Query("SELECT m FROM Member m WHERE (m.name LIKE %:keyword% OR m.email LIKE %:keyword%) AND (m.isDeleted = false OR m.isDeleted IS NULL) ORDER BY m.createdAt ASC, m.name ASC")
    Page<Member> searchByKeywordOrderByCreatedAt(@Param("keyword") String keyword, Pageable pageable);

    // 전체 회원 목록 + 가입일 과거순 정렬전용
    @Query("SELECT m FROM Member m WHERE m.isDeleted = false OR m.isDeleted IS NULL ORDER BY m.createdAt ASC, m.name ASC")
